...
export const OrderConfirmationModal: React.FC<OrderConfirmationModalProps> = ({
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-50 overflow-y-auto bg-black/85 backdrop-blur-md p-3 sm:p-6 flex items-center justify-center transition-opacity"
      onClick={onClose}
    >
      <div
        id="order-confirm-card"
        className="bg-[#091711] border border-[#224d3b] rounded-2xl max-w-xl w-full text-[#f6f3ed] shadow-2xl overflow-hidden my-6"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Banner with Success Confirmation */}
        <div className="bg-gradient-to-r from-[#15382a] via-[#0f271d] to-[#091711] p-6 border-b border-[#224d3b] text-center space-y-2">
        ...
        <div className="p-5 sm:p-7 space-y-5">
          ...
          <div className="bg-[#07130e] border border-[#224d3b] rounded-xl p-4 space-y-3">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              ...
              <div className="text-right sm:text-right">
                ...
                <span className="text-xs font-semibold text-[#f6f3ed]">+{RESTAURANT_WHATSAPP_NUMBER}</span>
              </div>
            </div>

            {/* Direct WhatsApp Order Submission Actions */}
            <div className="pt-2 border-t border-[#224d3b]/50">
              <div className="text-xs text-[#c8c0b2] mb-2 flex items-center justify-between">
              ...
              {showMessagePreview && (
                <div className="mt-3 p-3 rounded-lg bg-[#050d0a] border border-[#224d3b] text-left">
                  ...
                  <pre className="text-[11px] font-mono text-[#c8c0b2] whitespace-pre-wrap leading-relaxed select-all">
                    {whatsappMessage}
                  </pre>
                </div>
              )}
          ...
          <div className="bg-[#07130e] border border-[#224d3b] rounded-xl p-4 text-xs space-y-2.5">
            <div className="grid grid-cols-2 gap-3 text-[#c8c0b2] border-b border-[#224d3b]/60 pb-3">
              <div>
              ...
              <div>
                ...
                <span className="font-semibold text-[#d4af37] flex items-center gap-1">
                  ...
                  {isPickup && <PackageCheck className="w-3.5 h-3.5" />}
                  {order.orderType === 'dine_in' && <Utensils className="w-3.5 h-3.5" />}
            ...
            {isDelivery && order.deliveryAddress && (
              <div className="p-2.5 rounded-lg bg-[#0e241b] border border-[#224d3b] flex items-start space-x-2">
                ...
                <div>
                  <span className="font-semibold text-[#f6f3ed] block">Delivery Destination:</span>
                  <p className="text-xs text-[#c8c0b2]">{order.deliveryAddress}</p>
                </div>
              </div>
            )}
            ...
            <div className="space-y-1.5 pt-1">
              ...
              <div className="divide-y divide-[#224d3b]/30">
                {order.items.map((i, idx) => (
                  <div key={idx} className="py-1 flex justify-between items-center text-[#c8c0b2]">
                    <span>
          ...
          <div className="p-3 bg-[#15382a]/40 rounded-xl border border-[#224d3b] text-xs text-[#c8c0b2] space-y-1">
          ...
          <div className="flex items-center justify-between pt-1 gap-2">
            <button
              ...
              className="w-full py-3 rounded-xl bg-gradient-to-r from-[#d4af37] via-[#f1d779] to-[#c59b27] hover:brightness-110 text-[#091711] font-bold text-xs uppercase tracking-wider transition-all shadow active:scale-[0.98]"
            >
